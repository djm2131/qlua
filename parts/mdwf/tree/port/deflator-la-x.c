...
#define DEFLATOR_DIM(pstate) ((pstate)->lat_x->full_size)
...
qx(defl_vec_alloc)(struct Q(State) *state)
{
    ...
    res.mem_ptr = qx(allocate_xy_)(state, &res.mem_size,
                                  (void *)&res.f, 0, 1, 0);
    if (res.mem_ptr == NULL)
        res.f = NULL;
    return res;
...
void 
qx(defl_vec_copy)(struct Q(State) *state, qx(defl_vec) x, qx(defl_vec) y)
{
    assert(x.dim == y.dim);
    assert(x.Ls == y.Ls);
    assert(!defl_vec_is_null(&x));
    assert(!defl_vec_is_null(&y));
    qx(omp_f_copy)(state, y.f, y.dim, y.Ls, x.f);
}
void 
...
void
qx(defl_vec_zero)(struct Q(State) *state, qx(defl_vec) x)
{
    assert(!defl_vec_is_null(&x));
    qx(omp_f_zero)(state, x.f, x.dim, x.Ls);
}
void
...
int
qx(defl_vec_dotu)(struct Q(State) *state,
        doublecomplex *res,
        qx(defl_vec) x, 
        qx(defl_vec) y)
{
    ...
    double s[2];
    s[0] = s[1] = 0.;
    int flops = qx(omp_f_dot)(state, s, s + 1, x.dim, x.Ls, x.f, y.f);
...
int
qx(defl_vec_scal)(struct Q(State) *state, double alpha, qx(defl_vec) x)
{
    assert(!defl_vec_is_null(&x));
    return qx(omp_f_rmul1)(state, x.f, x.dim, x.Ls, alpha);
}

int 
qx(defl_vec_axpy)(struct Q(State) *state, double alpha, qx(defl_vec) x, qx(defl_vec) y)
{
    assert(x.dim == y.dim);
    assert(x.Ls == y.Ls);
    assert(!defl_vec_is_null(&x));
    ...
    return qx(omp_f_add2)(state, y.f, y.dim, y.Ls, alpha, x.f);
}

int
qx(defl_vec_nrm2)(struct Q(State) *state, double *res, qx(defl_vec) x) 
{
    assert(!defl_vec_is_null(&x));
    assert(NULL != res);
    
    int flops = qx(omp_f_norm)(state, res, x.dim, x.Ls, x.f);
    QMP_sum_double(res);
...
void 
qx(defl_mat_copy)(struct Q(State) *state, qx(defl_mat) m1, qx(defl_mat) m2)
{
    ...
    qx(omp_vf_copy)(state, m1.dim, m1.Ls, m1.len, 
...
int
qx(defl_mat_insert_col)(struct Q(State) *state, qx(defl_mat) m, int col, qx(defl_vec) v)
{
    assert(col < m.len);
    assert(v.dim == m.dim);
    assert(v.Ls == m.Ls);
    ...
    return qx(omp_vf_put)(state, m.dim, m.Ls,
			  m.fv, m.stride, col,
			  v.f);
}
int
qx(defl_mat_get_col)(struct Q(State) *state, qx(defl_mat) m, int col, qx(defl_vec) v)
{
    assert(col < m.len);
    assert(v.dim == m.dim);
    assert(v.Ls == m.Ls);
    ...
    return qx(omp_vf_get)(state, m.dim, m.Ls,
                      v.f,
                      m.fv, m.stride, col);
}
...
int 
qx(defl_lmH_dot_lm)(struct Q(State) *state, int m, int n,
		    qx(defl_mat) a, 
		    qx(defl_mat) b,
		    doublecomplex *c, int ldc)
{
    ...
    int flops = qx(omp_do_vfH_dot_vf)(state, a.dim, a.Ls,
				      (double *)c, ldc,
...
int 
qx(defl_lmH_dot_lv)(struct Q(State) *state, int m, 
		    qx(defl_mat) a, 
		    qx(defl_vec) x,  
		    doublecomplex *y)
{
    ...
    int flops = qx(omp_do_vfH_dot_f)(state, a.dim, a.Ls,
				     (double *)y, 
				     a.fv, a.stride, a.begin, a.len,
				     x.f);
...
int 
qx(defl_lm_dot_zm)(struct Q(State) *state, int n, int k, 
		   qx(defl_mat) a, 
		   ...
		   qx(defl_mat) c)
{
    ...
    return qx(omp_vf_dot_mz)(state, a.dim, a.Ls,
...
int
qx(defl_lm_dot_zv)(struct Q(State) *state, int n, 
		   qx(defl_mat) a, 
		   doublecomplex *x,
		   qx(defl_vec) y)
{
    ...
    return qx(omp_vf_dot_vz)(state, a.dim, a.Ls,
			     y.f,
			     a.fv, a.stride, a.begin, a.len,
			     (double *)x);
}
