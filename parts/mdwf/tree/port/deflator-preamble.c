        ...
        unsigned int               options)
{
    ...
    if (df->usize <= 0) {
        ...
        qx(defl_vec_copy)(df->state, lv_b, lv_r);
    } else {
        ...
        qx(defl_vec_copy)(df->state, lv_b, lv_r);
        fl += qx(defl_vec_axpy)(df->state, -1., ws_vec, lv_r);
    }
    
    fl += qx(defl_vec_nrm2)(df->state, r_norm2, lv_r);
    ...
    qx(defl_vec_copy)(df->state, lv_r, ws_vec);
    fl += qx(defl_vec_scal)(df->state, 1. / sqrt(*r_norm2), ws_vec);
    fl += qx(defl_mat_insert_col)(df->state, d_e->V, 0, ws_vec);
