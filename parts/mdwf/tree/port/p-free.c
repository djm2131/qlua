...
static void
free_ktable(struct Q(State) *state, struct KTable **table)
{
    int Ls = state->Ls;
    int ks = q(sizeof_KTable(Ls));

    if (*table == 0)
	return;
    q(free)(state, *table, ks);
    *table = 0;
}
...
Q(free_parameters)(struct Q(Parameters) **parameters_ptr)
{
  ...
  free_ktable(state, &(*parameters_ptr)->KTable);
  free_ktable(state, &(*parameters_ptr)->KiTable);
