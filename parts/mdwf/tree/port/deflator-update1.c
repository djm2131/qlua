...
#  error "no linear algebra library"
    ...
    fl += qx(defl_lm_dot_zm)(df->state, vsize, vmax, 
                             d_e->V,
                             d_e->hevecs2, vmax, 
                             tmp_V);
    qx(defl_mat_copy)(df->state, tmp_V, qx(defl_mat_submat_col)(d_e->V, 0, vsize));
    ...
    fl += qx(defl_lmH_dot_lv)(df->state, vsize, 
                              tmp_V, 
    ...
    /* V[:,vsize] <- ||resid|| */
    qx(defl_vec_copy)(df->state, qx(defl_vec_view)(df->state, resid), 
                      cur_r);
    fl += qx(defl_vec_scal)(df->state, 1. / resid_norm, cur_r);
    fl += qx(defl_mat_insert_col)(df->state, d_e->V, d_e->vsize, cur_r);
