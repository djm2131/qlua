...
void qx(defl_vec_zero)(struct Q(State) *state, qx(defl_vec) x); /* x <- 0 */
void qx(defl_vec_copy)(struct Q(State) *state, qx(defl_vec) x, qx(defl_vec) y); /* y <- x */
void qx(defl_vec_swap)(qx(defl_vec) *x, qx(defl_vec) *y); /* x <-> y */
/* return flop count */
int qx(defl_vec_dotu)(struct Q(State) *state, doublecomplex *res, qx(defl_vec) x, qx(defl_vec) y);
int qx(defl_vec_scal)(struct Q(State) *state, double alpha, qx(defl_vec) x);
int qx(defl_vec_axpy)(struct Q(State) *state, double alpha, qx(defl_vec) x, qx(defl_vec) y);
int qx(defl_vec_nrm2)(struct Q(State) *state, double *res, qx(defl_vec) x);
...
void qx(defl_mat_copy)(struct Q(State) *state, qx(defl_mat) m1, qx(defl_mat) m2); /* m2 <- m1 */
...
int qx(defl_mat_insert_col)(struct Q(State) *state, qx(defl_mat) m, int col, qx(defl_vec) v);
int qx(defl_mat_get_col)(struct Q(State) *state, qx(defl_mat) m, int col, qx(defl_vec) v);


/* C <- A^\dag * B, A:lat*m, B:lat*n, C:m*n */
#if 0 /* XXX */
int qx(defl_lmH_dot_lm)(struct Q(State) *state, int m, int n,
                        qx(defl_mat) a, 
                        qx(defl_mat) b, 
                        doublecomplex *c, int ldc);
#endif /* XXX */
/* y <- A^\dag x, A:lat*m, x:lat, y:m */
int qx(defl_lmH_dot_lv)(struct Q(State) *state, int m,
                        qx(defl_mat) a, 
                        qx(defl_vec) x, 
                        doublecomplex *y);
/* C <- A * B, A:lat*k, B:k*n, C:lat*n */
int qx(defl_lm_dot_zm)(struct Q(State) *state, int n, int k,
                       qx(defl_mat) a,
                       doublecomplex *b, int ldb, 
                       qx(defl_mat) c);
/* y <- A * x, A:lat*n, x:n, y:lat */
int qx(defl_lm_dot_zv)(struct Q(State) *state, int n,
                       qx(defl_mat) a, 
                       doublecomplex *x,
                       qx(defl_vec) y);
