...
#define MAX_OPTIONS (QOP_CLOVER_LOG_CG_RESIDUAL     | \
                     QOP_CLOVER_LOG_TRUE_RESIDUAL   | \
                     QOP_CLOVER_FINAL_CG_RESIDUAL   )

int
QX(MxM_CG)(struct QX(HalfFermion)       *psi,
           int                          *out_iterations,
           double                       *out_epsilon,
           const struct QX(HalfFermion) *psi_0,
           ...
           unsigned int                  options)
{
    ...
    flops += qx(f_norm)(&rhs_norm, e_size, eta->even);
    ...
    /* precondition */
    qx(f_copy)(chi_e, e_size, eta->even);
    /* reduce rhs */
    ...
    /* solve */
    cg_status = qx(cg_solver)(psi->even, "MxM CG", out_iterations, out_epsilon,
    ...
    if (options & (QOP_CLOVER_FINAL_CG_RESIDUAL | QOP_CLOVER_LOG_CG_RESIDUAL)) {
