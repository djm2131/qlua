...
/* replace all references to `dim' with these macro */
...
qx(defl_vec) 
qx(defl_vec_view)(struct Q(State) *state, struct FermionX *f)
{
    qx(defl_vec) res;
    res.dim     = DEFLATOR_DIM(state);
    res.f       = f;
        ...
        qx(defl_vec) y)
{
    ...
    double s[2];
    int flops = qx(f_dot)(s, s + 1, x.dim, x.f, y.f);
...
{
    assert(!defl_vec_is_null(&x));
    return qx(f_rmul1)(x.f, x.dim, alpha);
}
...
qx(defl_vec_axpy)(double alpha, qx(defl_vec) x, qx(defl_vec) y)
{
    ...
    return qx(f_add2)(y.f, y.dim, alpha, x.f);
}
...
qx(defl_vec_nrm2)(double *res, qx(defl_vec) x) 
{
    ...
    int flops = qx(f_norm)(res, x.dim, x.f);
    QMP_sum_double(res);
...
qx(defl_mat_alloc)(struct Q(State) *state, int ncol)
{
    ...
    res.len     = ncol;
    res.fv      = NULL;
    res.stride  = qx(strideof_vfermion)(res.dim);
    res.mem_ptr = q(allocate_aligned)(state, &res.mem_size, (void *)&res.fv, 0,
                                      qx(sizeof_vfermion)(res.dim, ncol));
    if (res.mem_ptr == NULL)
        res.fv = NULL;
               ...
               doublecomplex *c, int ldc)
{
    ...
    int flops = qx(do_vfH_dot_vf)(a.dim,
                                  (double *)c, ldc,
               ...
               doublecomplex *y)
{
    ...
    int flops = qx(do_vfH_dot_f)(a.dim,
                                 (double *)y, 
                                 a.fv, a.stride, a.begin, a.len,
                                 x.f);
