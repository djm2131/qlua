           ...
           struct FermionX *t1_o)
{
    ...
    if (options & QOP_TWISTED_LOG_TRUE_RESIDUAL) {
        true_res = qx(cg_true_residual)(xi_e, state, gauge, chi_e,
                                        flops, sent, received,
                                        t0_e, t1_e, t0_o);
    }
#define ITER_LOG (QOP_TWISTED_LOG_CG_RESIDUAL |    \
                  QOP_TWISTED_LOG_TRUE_RESIDUAL)
    switch (options & ITER_LOG) {
#undef ITER_LOG
    default:
        break;
    case QOP_TWISTED_LOG_CG_RESIDUAL:
        qx(zprint)(state, source,
                   "CG step %5d"
                   "  CG residual %11.4e",
                   iter, cg_res);
        break;
    case QOP_TWISTED_LOG_TRUE_RESIDUAL:
        qx(zprint)(state, source,
                   "CG step %5d"
                   "  true residual %11.4e",
                   iter, true_res);
        break;
    case QOP_TWISTED_LOG_CG_RESIDUAL | QOP_TWISTED_LOG_TRUE_RESIDUAL:
        qx(zprint)(state, source,
                   "CG step %5d"
                   "  CG residual %11.4e"
                   "  true residual %11.4e",
