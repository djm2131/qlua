...
int
qx(defl_eigcg_update1)(struct QX(Deflator)     *df,
                       double                   alpha,
                       double                   beta,
                       double                   alpha_prev,
                       double                   beta_prev,
                       double                   resid_norm_sq,
                       struct FermionX         *resid,
                       struct FermionX         *A_resid,
                       unsigned int             options)
{
    ...
    zheev_(&cV, &cU, &vmax, d_e->hevecs1, &vmax,
           ...
           d_e->rwork, &info, 1, 1);
    assert(0 == info);
    if (options & QOP_TWISTEDLOG_EIG_UPDATE1) {
        int i;
        for (i = 0; i < vmax; i++)
    ...
    if (options & QOP_TWISTED_LOG_EIG_UPDATE1) {
        int i, j;
