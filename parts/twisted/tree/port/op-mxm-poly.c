             ...
             const double poly_c[])
{
    ...
    struct FermionX *tmp_e1;
    int e_size;
    int i;
    struct FermionX *poly_ws_e[3];
    struct FermionX *poly_v0_e,
                    *poly_v1_e,
                    *poly_v2_e,
                    *aux;
    ...
    alloc_ptr = qx(allocate_eo)(state, &alloc_size, &aligned_ptr, 0, 3, 1);
    if (alloc_ptr == 0)
    ...
    BEGIN_TIMING(state);
    /* v0 <- c0 * v */
    qx(f_copy) (poly_v0_e, e_size, fermion->even);
    qx(f_rmul1)(poly_v0_e, e_size, poly_c[0]);
    /* v1 <- (a0 + b0*A) . v */
    qx(op_even_M) (tmp_e1, state, gauge, fermion->even,
                  &flops, &sent, &received, tmp_o);
    qx(op_even_Mx)(poly_v1_e, state, gauge, tmp_e1,
                  &flops, &sent, &received, tmp_o);
    qx(f_rmul1)(poly_v1_e, e_size, poly_b[0]);
    qx(f_add2) (poly_v1_e, e_size, poly_a[0], fermion->even);

    for (i = 1; i < poly_n ; i++) {
        /* v2 = A.v1 */
        qx(op_even_M)(tmp_e1, state, gauge, poly_v1_e,
                      &flops, &sent, &received, tmp_o);
        qx(op_even_Mx)(poly_v2_e, state, gauge, tmp_e1,
                      &flops, &sent, &received, tmp_o);
        /* v2 *= b */
        qx(f_rmul1)(poly_v2_e, e_size, poly_b[i]);
        /* v2 += a*v1 */
        qx(f_add2) (poly_v2_e, e_size, poly_a[i], poly_v1_e);
        /* v2 += c*v0 */
        qx(f_add2) (poly_v2_e, e_size, poly_c[i], poly_v0_e);
    ...
    if (NULL != result_prev) {
        ...
        qx(f_copy)(result_prev->even, e_size, poly_v0_e);
    }
