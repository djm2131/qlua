...
int
QX(alloc_half_fermion_matrix)(struct QX(HalfFermionMat) **hfm_ptr,
...
int QX(blas_view_half_fermion_matrix)(struct QX(HalfFermionMat) *hfm_ptr,
                                      int *nrow_loc,
                                      int *ncol,
                                      REAL **blas_ptr,
                                      int *blas_ld)
{
    ...
    if (NULL != nrow_loc)
        *nrow_loc = hfm_ptr->state->even.full_size;
    if (NULL != ncol)
        *ncol = hfm_ptr->m.len;
    if (NULL != blas_ptr)
